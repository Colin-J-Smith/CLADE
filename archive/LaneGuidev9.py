...
def create_lanes(lane_edges, frame):
    ...
    if len(center_fit) > 0:
        ...
        avg_center_line = (center_line[1] + center_line[3]) / 2
        AbsDistance_center = abs(avg_center_line - detection_lane)

        if (AbsDistance_center <= 2) and (avg_center_line > detection_lane):
            int_count += 1

    return right_line, left_line, center_line


def draw_lanes(frame, right_line, left_line, center_line, left_int, right_int, quad1_int, quad2_int, quad3_int, quad4_int):
    # draw lane lines
    ...
    if len(quad3_int) > 0:
        ...
        cv2.line(lane_image, (x1, y1), (x2, y2), [100, 0, 100], thickness)
    if len(quad4_int) > 0:
        x1, y1, x2, y2 = quad4_int.reshape(4)
        cv2.line(lane_image, (x1, y1), (x2, y2), [0, 100, 100], thickness)
    ...
    if len(horizontal_fit) > 0:
        for [x1, y1, x2, y2] in horizontal_fit:
            if 0 < y1 <= 120:
                ...
                cv2.line(line_image, (x1, y1), (x2, y2), (100, 100, 0), 2)
            elif 240 < y1 <= 360:
                quad3.append((x1, y1, x2, y2))
                cv2.line(line_image, (x1, y1), (x2, y2), (0, 100, 100), 2)
            else:
                quad4.append((x1, y1, x2, y2))
                cv2.line(line_image, (x1, y1), (x2, y2), (100, 0, 100), 2)
...
## counting lines entering and exiting
    detection_lane = 400

    avg_y = (quad4_int[1] + quad4_int[3])/2
    AbsDistance = abs(avg_y - detection_lane)

    if (AbsDistance <= 2) and (avg_y > detection_lane):
        int_count += 1
...
def main():
    ...
    for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        slope, left_int, right_int, quad1_int, quad2_int, quad3_int, quad4_int = create_intersection(intersection_edges, frame)
        lane_image = draw_lanes(frame, right_line, left_line, center_line, left_int, right_int, quad1_int, quad2_int, quad3_int, quad4_int)
        if state1 == 1:
