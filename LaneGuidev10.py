...
def navigation(frame, center_line, right_line, left_line):
    ...
    with open(logfile, "a") as f:
        print("C=", center_line, abs(center_line[0] - center_line[3]) > 50, "L_l=", left_line, "R_l=", right_line, file=f)
    if state1 == 0 and len(center_line) > 0 and abs(center_line[0] - center_line[3]) > 50 and \
...
def process_intersection(frame, intersection_vertices):
    ...
    processed_hsv = cv2.cvtColor(ROI, cv2.COLOR_BGR2HSV)
    lower_purple = np.array([140, 80, 120], dtype=int)
    upper_purple = np.array([160, 255, 210], dtype=int)
...
def create_intersection(intersection_edges, frame):
    ...
    if state1 == 2:
        ...
        if len(left_int) > 0 and left_int[1] > 150:
            state1 = 1
        elif len(quad3_int) > 0 and 240 < quad3_int[1] < 300:
            state1 = 1
        elif len(right_int) > 0 and right_int[3] > 150:
            state1 = 1
